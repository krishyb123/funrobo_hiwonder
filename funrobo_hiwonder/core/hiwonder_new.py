...
class HiwonderRobot:
    def __init__(self):
        if os.path.exists("/dev/rrc"):
            print("[INFO] Detected V5 Board")
            self.controller = HiwonderRobotV5()
            self.board = "V5"
        else:
            print("[INFO] Detected V3 Board")
            self.controller = HiwonderRobotV36()
            self.board = "V3.6"
    ...
    def set_joint_values(self, thetalist: list, duration=1, radians=False):
        ...
        """
        if self.board == "V3":
            # Ensure servo is registered for polling so get_servo works later
            # self.controller.register_servo(servo_id)
            pass
        self.controller.set_joint_values(thetalist, duration, radians)

    def get_joint_values(self):
        if self.board == "V3":
            pass
        return self.controller.get_joint_values()

    def shutdown_robot(self):
        if self.board == "V3":
            pass
        return self.controller.shutdown_robot()
